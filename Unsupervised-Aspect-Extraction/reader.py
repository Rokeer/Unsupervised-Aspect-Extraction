...
def create_vocab(domain, maxlen=0, vocab_size=0):
    assert domain in {'restaurant', 'beer', 'mvp', 'space'}
    source = 'datasets/preprocessed_data/'+domain+'/train.txt'
    ...
    vocab_file = codecs.open('datasets/preprocessed_data/%s/vocab' % domain, mode='w', encoding='utf8')
...
def read_dataset(domain, phase, vocab, maxlen):
    ...
    source = 'datasets/preprocessed_data/'+domain+'/'+phase+'.txt'
